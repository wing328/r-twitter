#' Create a new InvalidRequestProblem
#'
#' @description
...
#' @description InvalidRequestProblem Class
...
#' @export
InvalidRequestProblem <- R6::R6Class(
  ...
  public = list(
    ...
    #' To JSON String
    #'
    #' @return InvalidRequestProblem in JSON format
    #' @export
    toJSON = function() {
      InvalidRequestProblemObject <- list()
      if (!is.null(self$`detail`)) {
        InvalidRequestProblemObject[["detail"]] <-
          self$`detail`
      }
      if (!is.null(self$`status`)) {
        InvalidRequestProblemObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`title`)) {
        InvalidRequestProblemObject[["title"]] <-
          self$`title`
      }
      if (!is.null(self$`type`)) {
        InvalidRequestProblemObject[["type"]] <-
          self$`type`
      }
      if (!is.null(self$`errors`)) {
        InvalidRequestProblemObject[["errors"]] <-
          lapply(self$`errors`, function(x) x$toJSON())
      }
      for (key in names(self$additional_properties)) {
        InvalidRequestProblemObject[[key]] <- self$additional_properties[[key]]
      }

      InvalidRequestProblemObject
    },
    #' Deserialize JSON string into an instance of InvalidRequestProblem
    #'
    #' @description
    #' Deserialize JSON string into an instance of InvalidRequestProblem
    #'
    #' @param input_json the JSON input
    #' @return the instance of InvalidRequestProblem
    #' @export
...
',
    ...
    #'
    #' @description
    #' Validate JSON input with respect to InvalidRequestProblem and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`title`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for InvalidRequestProblem: the required field `title` is missing."))
      }
      # check the required field `type`
      if (!is.null(input_json$`type`)) {
        stopifnot(is.character(input_json$`type`), length(input_json$`type`) == 1)
      } else {
        stop(paste("The JSON input `", input, "` is invalid for InvalidRequestProblem: the required field `type` is missing."))
      }
    },
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of InvalidRequestProblem
    #' @export
    toString = function() {
      self$toJSONString()
    },
