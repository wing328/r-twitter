#' Create a new Get2UsersByResponse
#'
#' @description
#' Get2UsersByResponse Class
#'
#' @docType class
...
#' @export
Get2UsersByResponse <- R6::R6Class(
  "Get2UsersByResponse",
  public = list(
    `data` = NULL,
    `errors` = NULL,
    ...
    #' Initialize a new Get2UsersByResponse class.
    #'
    #' @description
    #' Initialize a new Get2UsersByResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return Get2UsersByResponse in JSON format
    #' @export
    toJSON = function() {
      Get2UsersByResponseObject <- list()
      if (!is.null(self$`data`)) {
        Get2UsersByResponseObject[["data"]] <-
          lapply(self$`data`, function(x) x$toJSON())
      }
      if (!is.null(self$`errors`)) {
        Get2UsersByResponseObject[["errors"]] <-
          lapply(self$`errors`, function(x) x$toJSON())
      }
      if (!is.null(self$`includes`)) {
        Get2UsersByResponseObject[["includes"]] <-
          self$`includes`$toJSON()
      }
      for (key in names(self$additional_properties)) {
        Get2UsersByResponseObject[[key]] <- self$additional_properties[[key]]
      }

      Get2UsersByResponseObject
    },
    #' Deserialize JSON string into an instance of Get2UsersByResponse
    #'
    #' @description
    #' Deserialize JSON string into an instance of Get2UsersByResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of Get2UsersByResponse
    #' @export
...
',
    ...
    #' Validate JSON input with respect to Get2UsersByResponse
    #'
    #' @description
    #' Validate JSON input with respect to Get2UsersByResponse and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of Get2UsersByResponse
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# Get2UsersByResponse$unlock()
#
## Below is an example to define the print fnuction
# Get2UsersByResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# Get2UsersByResponse$lock()
